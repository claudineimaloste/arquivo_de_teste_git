...
def criar_arquivo():
    ...
    arquivo_cadasto = "cadastro.txt"        
    salvar_arquivo =  os.path.join(os.getcwd(),arquivo_cadasto)    
    
    if not os.path.exists(salvar_arquivo):
       
        with open (salvar_arquivo, "a", encoding="utf-8"):
            pass
...
def verificar_cadastro(matricula):
    ...
    if not  os.path.exists(salvar_arquivo):
        return False
    with open(salvar_arquivo, 'r', encoding="utf-8") as usuario:
        cadastro = usuario.read()
        return f"Login: {matricula}\n" in cadastro
...
def confirmar_senha (senha, conf_senha,matricula):  
    if senha != conf_senha:
        ...
        with open (salvar_arquivo, "a", encoding="utf-8") as arquivo:
            arquivo.write(f"Login: {matricula}\nSenha: {senha}\n\n")
...
def cadastrar():
    ...
    if verificar_cadastro(matricula):
...
def login():
    ...
    if f"Login: {matricula}\nSenha: {senha}" in cadastro:
       Chama_tela_principal()
       janela.destroy()
...
def chamar_tela_cadastro():
    ...
    tk.Label(tela_cadastro, text="Cadastro de Usuário", 
             font=("Arial", 14, "bold"),fg='white',background='black').pack(pady=10)
...
def Chama_tela_principal():
    ...
    tela_principal.configure(background="black")
